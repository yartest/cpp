...
unsigned Bot::GetStartingRegionValue(const int& noRegion)
{
	unsigned result = 0;
	...
	for (const auto& element : startingRegionsreceived)
	{
		if (element == noRegion)
			continue;

		bool found = regions[noRegion].IsNeighbor(element);
		if (found)
			result++;
	}
	result *= 100;

	int superRegion = regions[noRegion].getSuperRegion();
	int countRegionsInSuperRegion = superRegions[superRegion];
	result += (100 - countRegionsInSuperRegion);
...
{
	std::map<int, unsigned> values;
	for (const auto& element : startingRegionsreceived)
	{
		unsigned value = GetStartingRegionValue(element);
		values.insert(std::pair<int, unsigned>(element, value));
	}
	...
	startingRegionsreceived.erase(std::remove(startingRegionsreceived.begin(), startingRegionsreceived.end(), result->first), startingRegionsreceived.end());
